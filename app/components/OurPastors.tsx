...
import { motion, Variants } from 'framer-motion'; // ✅ single import
import { pacifico } from '@/app/fonts';
import { pastors } from '../data/pastors';
/* ================= SAFE ANIMATION (NO TS ERROR) ================= */
...
const item: Variants = {
  ...
  visible: {
    ...
    transition: {
      duration: 0.6,
      ease: [0.25, 0.1, 0.25, 1], // ✅ FIXED (no string error)
    },
  },
};
  ...
  return (
    <section className="px-6 lg:px-20 py-10 lg:mt-10 -mt-10 mb-15 relative">
      ...
      <motion.div
        ...
        whileInView="visible"
        viewport={{ once: true }}
        className="grid gap-8 sm:grid-cols-2 lg:hidden"
      >
        {pastors.slice(0, 4).map((tour, index) => (
          ...
          >
            ...
            <div className="relative w-full h-[240px] overflow-hidden">
              <Image
                src={tour.heroImage}
                alt={tour.cardTitle || tour.title || "Tour image"} // ✅ FIXED
                fill
            ...
            <div className="p-5 flex flex-col items-center text-center gap-4">
              ...
              <Link href={`/pastors/${tour.slug}`}>
                <button className="px-5 py-2 rounded-lg bg-[#5c6f87] text-white text-sm hover:bg-black transition">
                  {/* ✅ FIXED subtitle safely */}
                  {('subtitle' in tour && tour.subtitle) ? tour.subtitle : 'Pastors post'}
                </button>
              </Link>
            </div>
          </motion.div>
      ...
      {/* ================= MOBILE VIDEO ================= */}
      {/* <div className="block md:hidden mt-16 mx-auto h-[750px] w-[105%] -ml-2 rounded-[32px] overflow-hidden border border-gray-200 shadow-lg relative">
        <div
          className="absolute inset-0 bg-cover bg-center scale-105"
          style={{ backgroundImage: "url('/hero-poster1.jpg')" }}
        />
        <video
          src="/images/pastors/video2.mp4"
          autoPlay
          muted
